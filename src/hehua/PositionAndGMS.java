...
public class PositionAndGMS {
	...
	public  PositionAndGMS(LBRMed lbr, ITaskLogger logger) {
		lbr_iiwa = lbr;
		log = logger;
		kukaController = (MedController) lbr.getController();
		lbr_iiwa = (LBR) kukaController.getDevices().toArray()[0];
	}

    public void doReferencing()
    {
        ...
        for (int i = 0; i < axisId.length; ++i)
        {
            ...
            if (!isMastered)
            {
                log.warn("Axis with axisId " + axisId[i] + " is not mastered, therefore it cannot be referenced");
            }
        ...
        if (allAxesMastered)
        {
            log.info("Perform position and GMS referencing with 5 positions");
    ...
    {
        log.info("Moving to position #" + (++positionCounter));

        PTP mainMotion = new PTP(position).setJointVelocityRel(joggingVelocity);
        lbr_iiwa.move(mainMotion);

        log.info("Moving to current position from negative direction");
        ...
        log.info("Moving to current position from positive direction");
    ...
    private void sendSafetyCommand()
    {
        ...
        {
            log.warn("Command did not execute successfully, response = " + result);
        }
    }
